...
shinyApp(
  ui = dashboardPage(
    ...
    sidebar <- bs4DashSidebar(id = "sidebarmain",
                              bs4SidebarMenu(
                                bs4SidebarMenuItem(text ="Anasayfa", tabName = "Main", newTab = FALSE,startExpanded = T,bs4SidebarMenuSubItem(text ="Ayrýntýlar",tabName = "Main2",newTab = FALSE,selected = T)),
                                
                                bs4SidebarMenuItem(text ="Gelir Analizleri",startExpanded = T,
                                                   # bs4SidebarMenuSubItem(text ="Saatlere Göre Yoðunluk",tabName = "Hourly_population",newTab = FALSE),
                                                   # bs4SidebarMenuSubItem(text ="Haftalýk Yoðunluk Oranlarý",tabName = "Weekly_Ratio",newTab = FALSE),
                                                   bs4SidebarMenuSubItem(text ="Aylýk Gelir",tabName = "Monthly_Income",newTab = FALSE), # a timeseries for total earnings monthly period timeline must be changable 
                                                   ...
                                                   bs4SidebarMenuSubItem(text ="Ýptal/Satýþ Oranlarý",tabName = "Canceled_ratio",newTab = FALSE), #Piechart for canceled / order ratio
                                                   ...
                                                   bs4SidebarMenuSubItem(text ="Ýndirim Kayýplarý",tabName = "Lost_Of_Discount_Analysis",newTab = FALSE) # in a infobox most discount product will be serve and a barplot with discount amounth 
                                ),
                                
                                bs4SidebarMenuItem(text ="Ürünlerin analizleri",tabName = "Products",startExpanded = T,
                                                   bs4SidebarMenuSubItem(text ="Toplam Satýþ Adetleri", tabName = "Total_amounth_for_each_product", newTab = FALSE), 
                                                   bs4SidebarMenuSubItem(text ="Toplam Satýþ Tutarlarý", tabName = "Total_income_for_each_product", newTab = FALSE),#it will be a barplot for total selling for each month
                                ...
                                bs4SidebarMenuItem("Restorant Analizleri",tabName = "Restourant",startExpanded = T,
                                                   bs4SidebarMenuSubItem(text ="Masalarýn Satýþ Durumlarý",tabName = "Tables_Incomes_ratio",newTab = FALSE),
                                                   ...
                                                   bs4SidebarMenuSubItem(text ="Personel Performanslarý",tabName = "Workers_performance",newTab = FALSE)
                                )
                              )
    ) ,
    boardbody<-dashboardBody(shinyauthr::loginUI("login",user_title = "Kullanýcý Adý",title = "Üye Giriþi",pass_title = "Parola"),
                             ...
                             fluidRow(
                               box(id="plotbox",title = uiOutput("boxtitle"),closable = F,width = 12,
                                   tabItems(
                                     tabItem(
                                       tabName = "Main2",
                                       fluidRow(box(id = "box1",title = "Saatlik Yoðunluklar",echarts4rOutput("Hourly_Popularity"),width = 12),
                                                box(id = "box2",title = "Haftalýk Yoðunluklar", echarts4rOutput("Weekly_Popularity"),width = 12)
                                       )
                                     ), 
                                     tabItem(
                                       tabName = "Daily_Income",
                                       box(title =  "Günlük Satýþ Gelirleri",fluidRow(column(9,echarts4rOutput("Last_Thirty_Days_timeseries")),column(3,uiOutput("time"))),width = 12)
                                       
                                     ),
                                     tabItem(
                                       tabName = "Monthly_Income",
                                       box(title="Aylýk Satýþ Gelirleri",echarts4rOutput("All_Time_Monthly_timeseries"),width = 12)# done
                                     ),
                                     tabItem(
                                       tabName = "Canceled_ratio",
                                       box(title="Ýptal-Sipariþ Oranlarý",echarts4rOutput("Cancel_order_pie"),width = 12)# done
                                     ...
                                     tabItem(
                                       ...
                                       box(title="Ýndirim-Satýþ Geliri" ,echarts4rOutput("Discount_loss_Analysis"),width = 12)# done
                                       
                                     ),
                                     tabItem(
                                       tabName = "Total_amounth_for_each_product", #done
                                       box(title = "Aylýk Satýþ Miktarlarý",fluidRow(column(9,echarts4rOutput("Each_Product_Earning_Total")),column(3,uiOutput("Product_select"))),width = 12)
                                       
                                       
                                     ),
                                     tabItem(
                                       tabName = "Total_income_for_each_product", #done
                                       box(title = "Aylýk Satýþ Gelirleri",fluidRow(column(9,echarts4rOutput("Each_Product_Earning_Total2")),column(3,uiOutput("Product_select2"))),width = 12)
                                       
                                       
                                     ),
                                     tabItem(
                                       tabName = "Order_deliver_time", #done
                                       box(title = "Sipariþlerin Ýletilme Süreleri",fluidRow(column(9,echarts4rOutput("Order_Deliver_Time_Bar_intervals")),column(3,uiOutput("Product_select3"))),width = 12)
                                     ...
                                     tabItem(
                                       ...
                                       box(title = "Masalarýn Gelir Miktarlarý",echarts4rOutput("Tables_Incomes"),width = 12)
                                     ),
                                     tabItem(
                                       tabName = "Most_popular_spots",
                                       box(title= "Masalarýn Kullanýlma Miktarlarý" ,echarts4rOutput("Tables_Popularity"),width = 12)
                                     ),
                                     tabItem(
                                       tabName = "Time_Consumer_Tables",
                                       box(title = "Masalarýn Kullaným Süreleri",echarts4rOutput("Tables_Spended_Times"),width = 12)
                                     ),
                                     tabItem(
                                       tabName = "Workers_performance",
                                       box(title = "Çalýþanlarýn Aldýklarý Sipariþ Miktarlarý",echarts4rOutput("Employee_performance"),width = 12)
                                     )
                                   )
                               )
                             )
  ...
  server = function(input, output) { 
    ...
    output$Thirty_Day_Income_Infobox2 <- renderbs4InfoBox({
      ...
      bs4InfoBox("Aylýk Kazanç",paste0(value," tl"), icon = icon("list"),
    ...
    output$Seven_Day_Income_Infobox3 <- renderbs4InfoBox({
      ...
      value<-sum(df$itemVal)
      bs4InfoBox("Haftalýk Kazanç",paste0(value," tl"), icon = icon("list"),
                 color = "warning", fill = TRUE
      )
    ...
    output$Mean_Order_Time <- renderbs4InfoBox({
      ...
      bs4InfoBox("Ortalama Sipariþ Süresi",paste0(value," Dakika"), icon = icon("list"),
                 color = "orange", fill = TRUE
      )
    ...
    output$Max_Order_Time <- renderbs4InfoBox({
      ...
      bs4InfoBox("Maximum Sipariþ Süresi",paste0(value," Dakika"), icon = icon("list"),
                 color = "orange", fill = TRUE
      )
    ...
    output$Employee_Of_The_Month <- renderbs4InfoBox({  
      ...
      dff[dff==""] <- NA
      dff<-drop_na(dff)
      bs4InfoBox("Ayýn Elemaný",paste0(dff$sentBy[1]," ","Ýþlem:",dff$n[1]), icon = icon("list"),
                 color = "warning", fill = TRUE
      )
    ...
    output$Cancel_order_pie <-renderEcharts4r({
      ...
      cnames<-c("Ýptal Sipariþ","Tamamlanan Sipariþ")
