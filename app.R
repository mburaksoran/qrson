...
user_base<-collection$find('{}')
    ...
    boardbody<-dashboardBody(shinyauthr::loginUI("login",user_title = "Kullanıcı Adı",title = "Üye Girişi",pass_title = "Parola",login_title = "Giriş"),
                             ...
                             fluidRow(
                               box(id = "infobox",closable = TRUE,width = 12,
                                   ...
                                   fluidRow(
                                     bs4InfoBoxOutput("Rest_name",width = 12),
                               ...
                               box(id="plotbox",title = uiOutput("boxtitle"),closable = F,width = 12,
                                   tabItems(
                                     ...
                                     tabItem(
                                       ...
                                       box(title =  "Günlük Satış Gelirleri",fluidRow(column(9,echarts4rOutput("Last_Thirty_Days_timeseries")),column(3,uiOutput("time"),bs4InfoBoxOutput("Intervals_incomes",width = 12))),width = 12)
  ...
  server = function(input, output) { 
    ...
    reader_order <- reactiveFileReader(intervalMillis = 1000, session = NULL,filePath = 
                                         "./Orders_Raw.csv", readFunc = read.csv,header = T,sep = ",",encoding="UTF-8",fill = T,row.names = NULL,stringsAsFactors = FALSE, quote = "")
    
    filtered_reader_df <- reactive({
      df <- reader_order() %>% filter(restName ==user_info()$name)
      df$itemVal<-as.numeric(df$itemVal)
      df$itemCount<-as.numeric(df$itemCount)
      df$discountPercentage<-as.numeric(df$discountPercentage)
    ...
    reader_corder <- reactiveFileReader(intervalMillis = 1000, session = NULL,filePath = "./Corders_Raw.csv", readFunc = read.csv,header = T,sep = ",",encoding="UTF-8",fill = T,row.names = NULL,stringsAsFactors = FALSE, quote = "")
    ...
    output$Thirty_Day_Income_Infobox2 <- renderbs4InfoBox({
      ...
      hafta<-week(Sys.time())
      yil<-year(Sys.time())
      df<- df%>% filter(year==yil,mnth==ay)
      value<-sum(df$itemVal,na.rm = T)
      value<-round(value,1)
      bs4InfoBox("Aylık Kazanç",paste0(format(value,big.mark=",",scientific=FALSE)," ₺"), icon = icon("fas fa-lira-sign"),
                 color = "danger", fill = TRUE
      )
    ...
    output$Daily_Income_Infobox1 <- renderbs4InfoBox({
      ...
      ay<-month(Sys.time())
      yil<-year(Sys.time())
      df<- df%>% filter(year==yil,mnth==ay,daynummonth==gun)
      value<-sum(df$itemVal,na.rm = T)
      value<-round(value,1)
      bs4InfoBox("Günlük Kazanç",paste0(format(value,big.mark=",",scientific=FALSE)," ₺"), icon = icon("fas fa-lira-sign"),
                 color = "danger", fill = TRUE
      )
    ...
    output$Seven_Day_Income_Infobox3 <- renderbs4InfoBox({
      ...
      hafta<-week(Sys.time())
      yil<-year(Sys.time())
      dff<- df%>% filter(year==yil,mnth==ay,day=="Pazartesi")
      if(length(dff$X) > 0){
      start<-as.POSIXct(str_split(dff$closedAt[which.min(Sys.time()-dff$closedAt)]," ")[[1]][1])
      ...
      bs4InfoBox("Haftalık Kazanç",paste0(format(value,big.mark=",",scientific=FALSE)," ₺"), icon = icon("fas fa-lira-sign"),
                 color = "danger", fill = TRUE
      )
     ...
     output$Mean_Table_Time <- renderbs4InfoBox({
      ...
      bs4InfoBox("Ortalama Hizmet Süresi",paste0(value," Dakika"), icon = icon("far fa-clock"),
                 color = "danger", fill = TRUE
      )
    ...
    output$Mean_Order_Time <- renderbs4InfoBox({
      ...
      bs4InfoBox("Ortalama Sipariş Süresi",paste0(value," Dakika"), icon = icon("far fa-clock"),
                 color = "danger", fill = TRUE
      )
    ...
    output$Max_Order_Time <- renderbs4InfoBox({
      ...
      bs4InfoBox("Maximum Sipariş Süresi",paste0(value," Dakika"), icon = icon("far fa-clock"),
                 color = "danger", fill = TRUE
      )
    ...
    output$Rest_name <- renderbs4InfoBox({
      req(credentials()$user_auth)
      rest<-user_info()$name
      bs4InfoBox(paste0(toupper(user_info()$name)),color = "danger", fill = TRUE,icon = icon("fas fa-utensils"))
    ...
    output$Most_Popular_Table <- renderbs4InfoBox({
      ...
      bs4InfoBox("En Popüler Masa",paste0("Masa=",value), icon = icon("fas fa-thumbs-up"),
                 color = "danger", fill = TRUE
      )
    ...
    output$Most_Popular_Item <- renderbs4InfoBox({
      ...
      bs4InfoBox("En Popüler Ürün",paste0(dff$item[1]), icon = icon("fas fa-thumbs-up"),
                 color = "danger", fill = TRUE
      )
    ...
    output$Employee_Of_The_Month <- renderbs4InfoBox({  
      ...
      dff[dff==""] <- NA
      dff<-drop_na(dff)
      bs4InfoBox("Ayın Elemanı",paste0(dff$sentBy[1]), icon = icon("fa-solid fa-user-tie"),
                 color = "danger", fill = TRUE
      )
    ...
    output$time <- renderUI({
      req(credentials()$user_auth)
      df<-filtered_reader_df()
      dff<-df %>% filter(closedAt>as.POSIXct(Sys.time())-2592000)
      if(length(dff$X)>0){
        start1=as.POSIXct(Sys.time())-2592000
        stop1=Sys.time()
      }else{
        stop1=max(df$closedAt,na.rm = T)
        start1=stop1-259200
      }
     
      dateRangeInput("date", label = h6("Tarih"),start = start1 ,end = stop1)
    ...
    observe({
      ...
      rvv$time<-rv$time <- input$date
      
    })
    
    
    
    output$Intervals_incomes <- renderbs4InfoBox({
      req(credentials()$user_auth)
      df<-filtered_reader_df()#restoran filtresi silinecek
      df <- df %>% filter(isPaid == "True") 
      dff<-df%>%filter(closedAt>rvv$time[1],closedAt<rvv$time[2])
      value<-sum(dff$itemVal)
      value<-round(value,1)
      bs4InfoBox("Satış Geliri",paste0(format(value,big.mark=",",scientific=FALSE)," ₺"), icon = icon("fas fa-lira-sign"),
                 color = "danger", fill = TRUE)
